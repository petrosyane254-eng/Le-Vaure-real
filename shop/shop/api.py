...
def bootstrap(request):
    ...
    for x in PageSection.objects.filter(active=True).order_by("page", "sort_order", "id"):
        page_sections.setdefault(x.page, []).append({
            ...
            "imagePosition": x.image_position, "imagePositionX": x.image_position_x, "imagePositionY": x.image_position_y,
            "imageZoom": x.image_zoom, "minHeight": x.min_height, "desktopHeight": x.desktop_height,
            "mobileHeight": x.mobile_height, "sectionWidth": x.section_width, "textPositionX": x.text_position_x,
            "textPositionY": x.text_position_y, "textAlign": x.text_align, "contentWidth": x.content_width,
            "titleFontSize": x.title_font_size, "titleFontSizeMobile": x.title_font_size_mobile,
            "bodyFontSize": x.body_font_size, "overlayOpacity": x.overlay_opacity, "sortOrder": x.sort_order,
        })
    return JsonResponse({
...
def register(request):
    ...
    # Registration data stays in the session until the email code is verified.
