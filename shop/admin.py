...
class ProductVariantInline(TabularInline):
    ...
    extra = 1
    fields = ("color", "size", "sku", "stock", "price", "image", "active")
...
# =========================================================
# NEXT.JS STOREFRONT CONTENT MANAGEMENT
# =========================================================

@admin.register(ProductColor, site=admin.site)
...
@admin.register(ProductSize, site=admin.site)
class ProductSizeAdmin(ModelAdmin):
    list_display = ("name", "sort_order")
    list_editable = ("sort_order",)


@admin.register(ProductVariant, site=admin.site)
class ProductVariantAdmin(ModelAdmin):
    list_display = ("product", "color", "size", "sku", "stock", "price", "active")
    list_filter = ("active", "color", "size")
    search_fields = ("product__name", "sku", "color__name", "size__name")
    list_editable = ("stock", "active")


@admin.register(Collection, site=admin.site)
...
class SiteSettingsAdmin(ModelAdmin):
    fieldsets = (
        ("Brand", {"fields": ("brand_name", "brand_tagline", "announcement")}),
        ("Global colors", {"fields": (
            "primary_color", "accent_color", "page_background_color", "soft_background_color",
            "text_color", "muted_text_color", "border_color",
        )}),
        ("Header & announcement", {"fields": (
        ...
        ("Footer", {"fields": (
            "footer_background_color", "footer_text_color", "footer_note",
            "footer_shop_title", "footer_help_title", "footer_company_title",
            "footer_payment_title", "footer_payment_labels", "footer_copyright_text",
        )}),
        ("Product cards", {"fields": ("product_card_image_ratio", "product_card_radius", "product_grid_gap")}),
        ("Support", {"fields": ("support_email", "support_phone", "default_shipping_text", "default_returns_text")}),
        ("Social", {"fields": ("instagram_url", "facebook_url", "tiktok_url")}),
    )
...
@admin.register(NavigationItem, site=admin.site)
class NavigationItemAdmin(ModelAdmin):
    list_display = ("label", "location", "url", "sort_order", "active")
    list_filter = ("location", "active")
    list_editable = ("sort_order", "active")


@admin.register(HomeHeroSlide, site=admin.site)
class HomeHeroSlideAdmin(ModelAdmin):
    list_display = ("title", "button_label", "text_position", "sort_order", "active")
    list_editable = ("sort_order", "active")
    fieldsets = (
        ("Copy", {"fields": ("eyebrow", "title", "subtitle")}),
        ("Images", {"fields": ("image", "mobile_image", "background_position")}),
        ("Primary CTA", {"fields": ("button_label", "button_url")}),
        ("Secondary CTA", {"fields": ("secondary_button_label", "secondary_button_url")}),
        ("Colors", {"fields": ("text_color", "button_background_color", "button_text_color", "overlay_color", "overlay_opacity")}),
        ("Layout", {"fields": ("text_position", "desktop_height", "mobile_height", "sort_order", "active")}),
    )


@admin.register(PromoBanner, site=admin.site)
class PromoBannerAdmin(ModelAdmin):
    list_display = ("title", "placement", "sort_order", "active")
    list_filter = ("placement", "active")
    list_editable = ("sort_order", "active")
    fieldsets = (
        ("Copy", {"fields": ("eyebrow", "title", "subtitle")}),
        ("Image", {"fields": ("image", "background_position")}),
        ("CTA", {"fields": ("button_label", "button_url")}),
        ("Colors", {"fields": ("text_color", "button_background_color", "button_text_color", "overlay_color", "overlay_opacity")}),
        ("Layout", {"fields": ("text_position", "desktop_height", "mobile_height", "placement", "sort_order", "active")}),
    )


@admin.register(EditorialBlock, site=admin.site)
class EditorialBlockAdmin(ModelAdmin):
    list_display = ("title", "placement", "image_side", "sort_order", "active")
    list_filter = ("placement", "active")
    list_editable = ("sort_order", "active")
    fieldsets = (
        ("Copy", {"fields": ("eyebrow", "title", "body")}),
        ("Image", {"fields": ("image", "image_side", "image_position")}),
        ("CTA", {"fields": ("button_label", "button_url")}),
        ("Colors", {"fields": ("background_color", "text_color", "muted_text_color")}),
        ("Placement", {"fields": ("placement", "sort_order", "active")}),
    )


@admin.register(StorefrontSection, site=admin.site)
class StorefrontSectionAdmin(ModelAdmin):
    list_display = ("key", "title", "sort_order", "active")
    list_editable = ("sort_order", "active")
    fieldsets = (
        ("Section", {"fields": ("key", "active", "sort_order")}),
        ("Copy", {"fields": ("eyebrow", "title", "body")}),
        ("CTA", {"fields": ("button_label", "button_url")}),
        ("Colors", {"fields": ("background_color", "text_color")}),
    )


@admin.register(StoreBenefit, site=admin.site)
class StoreBenefitAdmin(ModelAdmin):
    list_display = ("title", "icon", "sort_order", "active")
    list_editable = ("sort_order", "active")
    list_filter = ("icon", "active")


@admin.register(ShopPageSettings, site=admin.site)
class ShopPageSettingsAdmin(ModelAdmin):
    fieldsets = (
        ("Hero copy", {"fields": ("eyebrow", "title", "subtitle", "breadcrumb_home_label")}),
        ("Filter labels", {"fields": (
            "search_placeholder", "category_heading", "size_heading", "color_heading",
            "price_heading", "clear_filters_label", "all_products_label",
            "loading_text", "empty_text",
        )}),
        ("Visibility", {"fields": (
            "show_sidebar", "show_search", "show_category_filter", "show_size_filter",
            "show_color_filter", "show_price_filter",
        )}),
        ("Layout", {"fields": ("products_per_row",)}),
        ("Colors", {"fields": (
            "hero_background_color", "hero_text_color",
            "sidebar_background_color", "card_background_color",
        )}),
    )

    def has_add_permission(self, request):
        return not ShopPageSettings.objects.exists()

    def has_delete_permission(self, request, obj=None):
        return False


@admin.register(PageSection, site=admin.site)
class PageSectionAdmin(ModelAdmin):
    list_display = ("page", "key", "title", "layout", "sort_order", "active")
    list_filter = ("page", "layout", "active")
    list_editable = ("sort_order", "active")
    search_fields = ("key", "title", "body")

    fieldsets = (
        ("LIVE VISUAL EDITOR", {"fields": (
            "image", "desktop_height", "mobile_height",
            "image_x", "image_y", "image_zoom", "overlay_opacity",
            "content_x", "content_y", "content_width",
            "title_size", "mobile_title_size", "body_size", "text_align",
        )}),
        ("Placement", {"fields": ("page", "key", "layout", "active", "sort_order")}),
        ("Copy", {"fields": ("eyebrow", "title", "body")}),
        ("CTA", {"fields": ("button_label", "button_url")}),
        ("Colors", {"fields": (
            "background_color", "text_color", "muted_text_color",
            "button_background_color", "button_text_color",
        )}),
        ("Legacy / fallback", {"fields": ("image_position", "min_height"), "classes": ("collapse",)}),
    )

    class Media:
        css = {"all": ("shop/admin/page_section_editor.css",)}
        js = ("shop/admin/page_section_editor.js",)


@admin.register(JournalPost, site=admin.site)
class JournalPostAdmin(ModelAdmin):
    list_display = ("title", "author_name", "published_at", "featured", "active")
    list_filter = ("featured", "active", "published_at")
    list_editable = ("featured", "active")
    prepopulated_fields = {"slug": ("title",)}
    search_fields = ("title", "excerpt", "body")


@admin.register(CustomerAddress, site=admin.site)
class CustomerAddressAdmin(ModelAdmin):
    list_display = ("user", "label", "city", "phone", "is_default")
    list_filter = ("is_default", "country", "city")
    search_fields = ("user__email", "full_name", "phone", "street")


@admin.register(PasswordResetCode, site=admin.site)
class PasswordResetCodeAdmin(ModelAdmin):
    list_display = ("user", "expires_at", "attempts", "last_sent_at")
    readonly_fields = ("code_hash", "last_sent_at")


@admin.register(OrderVerificationCode, site=admin.site)
class OrderVerificationCodeAdmin(ModelAdmin):
    list_display = ("order", "expires_at", "verified_at", "attempts")
